...
- **Working branch — only when isolation is actually needed** (Step 0.4): if `BASE_BRANCH` is `main` (or another shared/long-lived branch), create `iterate-fat-llama/<UTC timestamp>` (e.g. `iterate-fat-llama/20260905-141500`) and switch to it, so cycles — including a possible rollback's `git reset --hard` — never touch a branch other people/processes depend on. In the common case — `BASE_BRANCH` is already a single-purpose, disposable branch (anything `issue-branch-resolve.yml` checks out, or any other issue-linked branch a human hands you) — skip this and operate directly on `BASE_BRANCH`; a second branch there only adds confusing proliferation with no safety benefit, since the whole branch exists solely for this work already.
- Cycle tags: `<branch>/iter-<i>`, namespaced under whichever branch you actually ended up operating on (`BASE_BRANCH` itself, or the isolated working branch) so repeated runs don't collide; `iter-0` is the untouched starting state.
- Final branch/PR (Step 7): if you created an isolated working branch above, rename it to `v-<new_version>` before pushing/opening the PR, matching this repo's existing convention (e.g. `v-1.3.0`, `v-1.2.0.1`). If you stayed on `BASE_BRANCH` directly, there's no rename — push `BASE_BRANCH` itself and open the PR from it; the PR title still follows the `v-<new_version>` convention regardless of which branch it's opened from.
- **This is a branch-naming convention only — never create a git tag in this hyphenated form (`v-<version>`).** Release tags (created by the separate `github-release.yml` CI workflow after a merge to `main` reaches PyPI, not by this skill) use the unhyphenated `v<version>` form instead (e.g. `v1.4.2`), specifically so the two namespaces never collide — `git push`/`checkout` become ambiguous the moment a tag and a branch share one exact name, which is exactly what happened when `v-1.4.2`/`v-1.0.2.1`-style branches from past runs collided with an attempt to tag those same versions the same way. If this skill's own git-tag step (`<branch>/iter-<i>` cycle checkpoints) is ever extended to also tag a release, it must use the unhyphenated form to stay out of the branch-naming namespace above.
...
- **When staying on `BASE_BRANCH` directly (no isolated working branch), the final Step 7 push must be `git push --force-with-lease origin BASE_BRANCH`, not a plain push.** Intermediate commits can already be on the remote before Step 7 — `generate-code`'s own cycle checkpoints, and any commit `audio-quality-checker`'s remote-GPU path already pushed mid-run to validate against GitHub's API — so if step (f) rolled back to an earlier candidate, your final local history no longer descends from what's already there and a plain push would be rejected (or, with a bare `--force`, could silently clobber a genuinely unexpected remote change, e.g. a human pushing to this same branch mid-run). `--force-with-lease` gets you the rewrite you actually need while still refusing if the remote moved in a way you didn't account for.
...
2. **Step 0.4 (working branch):** the issue-linked branch `issue-branch-resolve.yml` checks out is always already single-purpose and disposable — this is exactly the common case in Naming conventions above, so never create a separate `iterate-fat-llama/<timestamp>` branch in CI mode; stay on it directly.
3. **Step 7 confirmation:** skipped — proceed directly through commit → push (force-with-lease, per Remote actions above, since you're on `BASE_BRANCH` directly) → open PR, exactly as if the user had already confirmed. State this plainly in the final report so it's auditable (e.g. "CI mode: Step 7 confirmation bypassed per `.claude/skills/rules/iterate-fat-llama.md`").
4. **PR body:** when `ITERATE_FAT_LLAMA_ISSUE_NUMBER` is set, the PR body (Step 7) must include a `Closes #<N>` line, in addition to the normal changelog-entry content, so GitHub auto-links and auto-closes the issue on merge. This is the only mechanism the second workflow (`issue-release-comment.yml`) has for finding its way back to the right issue later — don't omit it.
