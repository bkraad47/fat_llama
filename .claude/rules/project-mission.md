...
## Hard constraint: no content above the original Nyquist frequency

fat_llama's upscale pipeline must never leave (or reintroduce) audible spectral content above the original source file's Nyquist frequency (`original_sample_rate / 2`) in its output — whether that's zero-order-hold imaging, IST-injected harmonics, filter artifacts, or anything else. The goal for that band is silence, not synthesized/reconstructed detail: `upscale()` is a precision/headroom/format-upscale within the original recording's real bandwidth, not bandwidth extension above it. Any pipeline stage is free to compute intermediate content in that band, but the final output must have it removed — e.g. via an explicit FFT-domain cutoff/lowpass step (zero FFT bins above the original Nyquist, inverse-transform) applied after all other processing, still CUDA/CuPy-only and consistent with the DSP/IST-FFT method below. Treat any measured energy above the original Nyquist in produced audio as a defect to fix, not a coherence/spectral-accuracy tradeoff to weigh — `audio-quality-checker` should score it accordingly per its own rules file.
