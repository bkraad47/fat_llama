...
3. **Coherence score** — run the actual example pipeline and score the result per **Scoring → Coherence score** below, using exactly one of these two configs — **never both, and never anything you invent yourself**:
   ...
   **How to actually run it — remote GPU first, local GPU fallback, then give up loudly:** the `fllm-mcp-server` MCP server (tools: `mcp__fllm-mcp-server__test_generate`, `mcp__fllm-mcp-server__get_test_generate_status`) can run this on an on-demand remote GPU instead of your own environment. Follow this order every time you need to produce `output_test.flac`:
   ...
   1. **Try the remote path first.**
      ...
      e. On `succeeded`: check `push_status` before touching anything else — it is **not** implied by job `status`.
         - `push_status: "succeeded"` (with `result_commit_sha` set): the job committed the output file back onto that branch and pushed it — the worker always produces a real commit here, using `--allow-empty` when the bytes happen to be identical to what was already committed, so a `"succeeded"` push always means a fresh commit exists. Run `git fetch` then `git checkout`/`reset --hard` your local branch to `result_commit_sha` — another git-command exception, needed only to receive the remote job's own output, never to author new local changes — so `output_test.flac` matches what the remote run produced, and continue your analysis against that file as normal.
         - Any other `push_status` (`"failed"`, `"not_attempted"`, or `result_commit_sha` missing/`null`): **this is a run failure, not a soft skip — do not improvise a workaround.** Specifically, do not fall back to comparing file hashes, do not reuse whatever `output_test.flac` happens to already sit in the working tree, and do not silently treat it as equivalent to a successful push — you have no verified evidence that file corresponds to this run. Log the `push_status`/`error`/`error_description` verbatim, then stop: report exactly one failing entry in `tests` (`"type": "code"`, `"name": "mcp_push_verification"`, `"status": "fail"`, `"failure_reason"` citing the exact `push_status` and `error_description`), omit `scores`/`spectrogram_image` from your output entirely (this is what triggers `test-fat-llama`'s "missing scores" handling — leaves `README.md` untouched rather than writing partial data), and do **not** fall back to local generation (step 2) either — the remote pipeline output exists but this run couldn't verify it landed on the branch, and a local run wouldn't fix or explain that; it would just mask the failure with a state nobody else can reproduce.
      f. On `failed` or `timed_out` (job `status` itself, not `push_status`): read the stage-tagged `error`/`error_description` (clone / commit-mismatch / deps / CUDA / script / push) from the status response, log it, and fall back to local (step 2) — don't retry the remote path yourself.
