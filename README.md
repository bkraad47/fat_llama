...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 8 | Clean on every hygiene check against input_test.flac (0 NaN/Inf; clipping fraction 1.693e-04, identical to the reference's; 0 dropout runs >50ms; discontinuity 99.9th percentile matches the reference almost exactly; noise-floor rise is now 0.00 dB, so the +6.12 dB IST-driven rise from an earlier cycle is gone), but there is no measurable added detail — the out-vs-reference delta is 0.00 dB in all 32 bands from 0-154.35 kHz. |
| Spectral deviation vs. reference FLAC (0-10) | 10.0 | convergence=1.0000, correlation=1.0000 against input_test.flac (both 308700 Hz, no resampling, trimmed to 4672878 frames) — a degenerate perfect score, since input_test.flac is itself a 7x zero-order-hold of input_test.mp3 (max abs diff to this run's output is only 2.15e-06), not an independent high-resolution master. |
