...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 7 | Clean on every hard check - above-Nyquist energy ratio 8.83e-15, clip fraction 1.28e-6, 0 dropouts >50ms, 0 NaN/Inf, discontinuity p99.9 well below the reference's, spectral correlation 0.9919 with pitch/rhythm/structure intact - but a measurable non-source constant 98.168 Hz tone at -28.7 dBFS (+27.90 dB over reference at that frequency) collapses dynamic range from 57.1 dB to 25.4 dB. The previous cycle's fix moved IST's harmonic term out of the subsonic band but it is computed once from a whole-~15s-buffer FFT, so the "content-derived" frequency is the same on every iteration - a static drone, not time-varying detail. |
| Spectral deviation vs. reference FLAC (0-10) | 9.3 | convergence=0.8734, correlation=0.9919 against input_test.flac (resampled 176400->308700 Hz, trimmed to 4,672,878 samples); up from 9.0 last cycle - the subsonic artifact is gone (measured within 0.4 dB of the reference's own sub-20Hz content), but the relocated tone still caps this below the ~9.9 predicted if the harmonic term were removed entirely. |
