...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 6 | Structure is fully intact (STFT correlation 1.0000 and 20 ms RMS envelope correlation 1.0000 against input_test.mp3, spectral convergence 0.9909), with 0 invalid samples, negligible clipping (0.0228%), no click-level discontinuities (99.9th-percentile jump ratio 1.48 after scaling for the 7x upsample, max jump 0.0853) and duration preserved to +0.00028 s at 44100x7 = 308700 Hz stereo — but it falls short of a clean 8-9 because there is no measurable added detail (rolloff fell from 13717 Hz to 13415 Hz; the 2.46% ultrasonic energy is zero-order-hold imaging at 44/88/132 kHz, not reconstruction), the median broadband noise floor rose 1.43x (+3.1 dB), and a 204 ms LMS warm-up dropout at -82 dBFS opens the file. |
| Spectral deviation vs. reference FLAC (0-10) | 9.9 | convergence=0.9861, correlation=0.9999 against input_test.flac; both metrics agree that output_test.flac is essentially the same wave as the reference (both 308700 Hz stereo, 4672878 frames, no resampling needed), which is expected since the reference is itself a prior output of this same deterministic pipeline and therefore does not independently validate fidelity to the MP3 source. |
