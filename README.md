...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 8 | Clean on every artifact check - no NaN/Inf, clipping fraction 5.34e-5 (below the reference's own 2.29e-4), zero >50ms dropouts, 99.9th-percentile sample-to-sample jump 0.269x the reference's, and above-original-Nyquist content at -147.9 dB peak / -140.8 dB energy (FFT noise floor) - but no measurable added detail below 22.05 kHz (output matches its mp3 source within 0.0-0.3 dB in every band to 19 kHz), plus a non-source 0.066 Hz sine at -26.2 dBFS injected by IST's harmonic-reconstruction term (one cycle spans the entire ~15s buffer, landing subsonic/inaudible rather than as detail), so a safe/transparent upscale with one identified artifact rather than a 10. |
| Spectral deviation vs. reference FLAC (0-10) | 9.0 | convergence=0.8165, correlation=0.9830 against input_test.flac (output resampled 176400->308700 Hz, trimmed to 4,672,878 samples); the two metrics disagree because the convergence term is dominated almost entirely by the same 0.066 Hz IST injection - removing that one component raises convergence to 0.9839 and correlation to 0.9999 (score 9.9). |
...
4. **Iterative Soft Thresholding (IST)**: IST is applied to enhance the audio by adding missing frequencies. This process uses FFT to transform the signal into the frequency domain, apply a threshold to keep significant frequencies, and then inverse transform back to the time domain, adding a harmonic-reconstruction term each iteration whose amplitude is scaled to the signal's own peak and whose frequency is derived from the signal's own dominant retained frequency component each iteration (its first overtone, one octave up) rather than a fixed tone — so the added detail tracks what the audio actually contains and lands in the audible range instead of at one fixed (potentially inaudible) frequency regardless of content.
