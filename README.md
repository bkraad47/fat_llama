...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 9 | Clean on every check - 0 NaN/Inf, clipping fraction 9.0e-6 (below the reference's own 1.69e-4), 0 dropouts >50ms, 99.9th-pct sample-to-sample jump 0.0157 vs reference 0.0582 (ratio 0.269, smoother not clickier), and above-original-Nyquist (22050 Hz) content at -148.9/-138.9 dB peak and -186.9/-186.6 dB rms relative to in-band peak (FFT noise floor); high-band energy is measurably up below Nyquist (+1.10 dB at 12-16k, +2.02 dB at 16-19k, +2.98 dB at 19-22.05k) with low/mid bands flat within +/-0.5 dB - a HF tilt rather than a broadband noise-floor rise, held at 9 rather than 10 since the reference is itself a legacy-pipeline artifact so this isn't conclusively genuine reconstructed detail. |
| Spectral deviation vs. reference FLAC (0-10) | 9.0 | convergence=0.8165, correlation=0.9830 against input_test.flac (mono downmix, output resampled 176400->308700 Hz, trimmed to 4,672,878 samples, STFT nperseg=2048/noverlap=1024, each spectrogram max-normalized); the two metrics agree in direction, with convergence pulled down mainly by the modest high-band level difference. |
