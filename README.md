...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 9 | Clean on every artifact check (clipping fraction 5.1e-05, zero unmatched >50ms dropouts, zero NaN/Inf, discontinuity p99.9 0.0165 vs reference 0.0582 — smoother, not clickier) with measurable band-selective detail recovered in the congested top octaves (+1.94 dB 10-16 kHz, +3.21 dB 16-19 kHz, +3.73 dB 19-22.05 kHz) and no broadband noise rise; held below 10 because that lift tracks the interpolation's own sinc-compensation curve rather than being new content, nothing is added above the original 22.05 kHz Nyquist (-136 dB there), and IST's harmonic term currently injects a 0.0661 Hz subsonic drift (4.87% of peak) instead of audible detail. |
| Spectral deviation vs. reference FLAC (0-10) | 9.0 | convergence=0.8166, correlation=0.9830 against input_test.flac; the two metrics disagree because correlation sees near-identical in-band time-frequency structure while the Frobenius residual is dominated by the reference's own zero-order-hold mirror images (~44.1/88.2/132.3 kHz) that the new bandlimited output correctly lacks — input_test.flac is itself a legacy pipeline output, an artifact-bearing reference that caps the achievable convergence. |
