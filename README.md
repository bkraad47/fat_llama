...
| Metric | Score | Notes |
|---|---|---|
| Coherence (upscale quality, 0-10) | 6 | Clean on every hard check (NaN=0/Inf=0, 0 dropouts, clipping 1.69e-4 exactly equal to the input's so inherited not introduced, 99.9th-pct discontinuity 5.82e-2 below the input's 1.06e-1) and the audible band is fully intact at 0.9959 PSD shape correlation, but there is no genuine added audible detail — all energy above the input's 14352 Hz cutoff is zero-order-hold imaging replicas at multiples of 44.1 kHz (0.95-0.97 correlation with baseband) — alongside a +6.12 dB broadband in-band noise-floor rise. |
| Spectral deviation vs. reference FLAC (0-10) | 10.0 | convergence=0.998223, correlation=0.999998 against input_test.flac (both already 308700 Hz, n=4672878, no resampling needed); the two metrics agree, but this near-perfect score reflects that input_test.flac is itself a prior pipeline output (waveform r=0.9999986 vs this run's output) rather than an independent ground-truth master. |
