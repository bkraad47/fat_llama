...
**Kind:** function
**Description:** Runs `max_iter` rounds of: FFT the thresholded signal, zero out FFT bins at/below `threshold`, inverse-FFT back to the time domain, then add a sinusoidal harmonic term each iteration — used to reconstruct missing high-frequency content after upscaling. The harmonic term's *total* amplitude across all iterations is `0.1 * peak / max_iter` (cycle 2 bounded it to `0.1/max_iter` so it doesn't grow with `max_iter`; cycle 3 additionally scaled that total by `data`'s own peak amplitude, since `data` is raw-PCM-scale here and a fixed absolute `0.1` was measured to be an ~1e-5 relative contribution). **Issue #20 fix, cycle 3:** the harmonic term's *frequency* used to be a fixed `cp.sin(cp.linspace(0, 2*pi, len(data_thres)))` — exactly one cycle across the whole buffer regardless of length or content, landing at a subsonic ~0.066 Hz for a real ~15s upscaled buffer (measured: a non-source -26.2 dBFS artifact, `audio-quality-checker`'s sole remaining spectral-deviation driver). Now derives the harmonic's frequency each iteration from the dominant non-DC bin of that iteration's own already-computed masked FFT (`data_fft_thres`) and injects its first overtone (`2x` that bin index, clamped below this buffer's own Nyquist bin, with a mid-band fallback bin for the degenerate all-zero case) — a standard harmonic-exciter DSP technique, so the injected content tracks the signal's own dominant frequency instead of a fixed subsonic tone. Any overtone content this pushes above the *original* source's Nyquist frequency is still removed by the pipeline's separate, unconditional `apply_original_nyquist_cutoff` stage downstream — this fix adds detail within the original bandwidth, it does not extend it. **Known issue (investigated, not fixed as of cycle 3):** `threshold` is applied as an absolute cutoff to both raw-PCM-scale time-domain samples and raw FFT-bin magnitudes, but real audio's FFT-bin magnitudes are order ~1e4-1e5 — many orders of magnitude above the conventional default `threshold=0.6` — so the "keep significant frequencies, discard noise" masking barely triggers at real audio scale, leaving IST to mostly perform a near-lossless FFT/IFFT round trip beyond the harmonic term. Flagged as a strong next-cycle candidate (convert to a peak-relative fraction).
**Parameters:**
...
**File:** fat_llama/audio_fattener/feed.py:330
**Kind:** function
...
**File:** fat_llama/audio_fattener/feed.py:359
**Kind:** function
...
**File:** fat_llama/audio_fattener/feed.py:514
**Kind:** function
...
**File:** fat_llama/audio_fattener/feed.py:546
**Kind:** function
...
**File:** fat_llama/audio_fattener/feed.py:559
**Kind:** function
...
**File:** fat_llama/audio_fattener/feed.py:673
**Kind:** function
...
#### `TestAudioFattener.test_ist_harmonic_term_lands_in_audible_band(self) -> None`
**File:** fat_llama/tests/test_feed.py:622
**Kind:** method
**Description:** Added in cycle 3 (issue #20 fix) as a regression test for the harmonic-reconstruction-frequency fix. Isolates the harmonic term the same way `test_ist_harmonic_amplitude_scales_with_signal_peak` does (negligible threshold, near-identity FFT/IFFT round trip) on two synthetic two-tone signals (300/700 Hz and 100/900 Hz) and asserts the added content's dominant frequency is `>20 Hz` (not the old bug's fixed ~1 Hz-equivalent subsonic injection) and tracks `2x` each input's own dominant frequency (600 Hz and 200 Hz respectively, within 5 Hz), rather than sitting at one fixed frequency regardless of content.
**Returns:** `None` — raises `AssertionError` on failure via `unittest` assertions.

#### `TestAudioFattener.test_new_interpolation_algorithm_is_bandlimited(self) -> None`
**File:** fat_llama/tests/test_feed.py:692
**Kind:** method
...
**File:** fat_llama/tests/test_feed.py:747
**Kind:** method
...
**File:** fat_llama/tests/test_feed.py:830
**Kind:** method
...
**File:** fat_llama/tests/test_feed.py:905
**Kind:** method
...
**File:** fat_llama/tests/test_feed.py:1002
**Kind:** method
...
- `factblock_stale` / inconsistencies observed while reviewing (not fixed by this skill — report only):
  ...
  - `iterative_soft_thresholding`'s harmonic-reconstruction term used to span exactly one sine cycle across the *entire* buffer regardless of sample rate (`cp.sin(cp.linspace(0, 2*pi, len(data_thres)))`) — for a real ~15s buffer that's a ~0.066 Hz subsonic oscillation, not audible-band content (found in cycle 4 by `audio-quality-checker`: 4.87% of peak amplitude concentrated there, sub-20Hz energy at 3.32% of total output energy vs. 0.018% in the reference; re-measured in this run's cycle 3 at -26.2 dBFS, the sole remaining driver of the spectral-deviation score). An initial cycle 4 attempt to fix this by extrapolating real content into the band above the original Nyquist was **redirected by the user mid-run** — the project's design goal changed: that band must be kept silent (see the project-mission.md hard constraint and `apply_original_nyquist_cutoff` above), not filled with reconstructed detail. **Fixed in this run's cycle 3** (issue #20 fix): the harmonic's frequency is now derived each iteration from the dominant retained FFT bin (its first overtone), landing in the audible band and tracking the signal's own content instead of a fixed subsonic tone — see `iterative_soft_thresholding`'s factblock above. Real-GPU confirmation that this measurably improves `audio-quality-checker`'s coherence/spectral-deviation scores (vs. the numpy-shim verification `generate-code` used) is still open as of cycle 3.
